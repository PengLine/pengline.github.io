...
def update_file(file_path):
    ...
    try:
        ...
        # 替换登录状态检查函数
        # 注意：这个正则表达式可能过于复杂，暂时注释掉
        # content = re.sub(r'document\.addEventListener\(\'DOMContentLoaded\', \(\) => \{[\s\S]*?checkLoginStatus\([\'\"]admin[\'\"]\);[\s\S]*?document\.getElementById\([\'\"]logout-btn[\'\"]\)\.addEventListener\([\'\"]click[\'\"]\, \(\) => \{[\s\S]*?logout\(\);[\s\S]*?\}\);[\s\S]*?displayCurrentUser\(\);[\s\S]*?\}\);', 
